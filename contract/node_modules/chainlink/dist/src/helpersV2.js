...
async function assertActionThrows(action) {
    ...
    const hasErrored = ERROR_MESSAGES.reduce((prev, next) => prev || e.message.includes(next), false);
...
exports.decodeDietCBOR = (data) => {
    return cbor_1.default.decodeFirstSync(addCBORMapDelimiters(data));
};
/**
...
async function fulfillOracleRequest(oracleContract, runRequest, response, options = {
    ...
    return oracleContract.fulfillOracleRequest(runRequest.id, runRequest.payment, runRequest.callbackAddr, runRequest.callbackFunc, runRequest.expiration, response, options);
}
exports.fulfillOracleRequest = fulfillOracleRequest;
function requestDataBytes(specId, to, fHash, nonce, data) {
    const types = [
        'address',
        'uint256',
        'bytes32',
    ...
    const encoded = ethers_1.ethers.utils.defaultAbiCoder.encode(types, values);
    const funcSelector = ethers_1.ethers.utils.id('oracleRequest(address,uint256,bytes32,address,bytes4,uint256,uint256,bytes)');
...
//# sourceMappingURL=helpersV2.js.map